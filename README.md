...
- -c, --correction "калибровочная" поправка в градусах, по умолчанию 0.0, или строка "calibrate"

Если задана опция -c calibrate, программа перед началом работы по три раза считывает значения температуры
инфракрасного датчика и температуры корпуса датчика (окружающего воздуха), разница между этими температурами
используется в качестве поправки.

Если разница между температурами инфракрасного датчика и корпуса датчика (окружающего воздуха) больше 10 градусов,
программа завершает работу с кодом -515.

В случае появления ошибки -515 перезапустите программу, или задайте значение поправки.

Дробная часть значения вводится отделяется о целой части точкой.
...
-  -N, --no-reconnect        не пробовать переоткрыть COM порт
-  -w, --wait                ждать CRLF
...
Устройство последовательного порта Attiny85 при закрытии соединения с ним при повторном открытии может перестать работать.

Отсоедините USB порт, включите заново, подождите поминуты. Запустите программу заново.
